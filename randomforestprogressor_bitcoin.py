...
# Fungsi untuk mengambil data historis BTC/USDT
def fetch_btc_data(exchange, symbol='BTC/USDT', timeframe='1h', limit=100):
    try:
...
def main():
    ...
    for i, (price, tp_val, sl_val) in enumerate(zip(predicted_prices, tp, sl)):
        print(f"{i+1:<5} {price:<10.2f} {tp_val:<10.2f} {sl_val:<10.2f}")
